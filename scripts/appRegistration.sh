...
CLIENT=__client__
API=__api__
RG=__resourceGroup__
WEBURL=__staticWebURL__

# Create Client App
CLIENT_APP=$(az ad app create --display-name "${CLIENT}" --identifier-uris "http://${CLIENT}")
...
# Create API App
API_APP=$(az ad app create --display-name "${API}" --identifier-uris "http://${API}")
# Get App Id and Object Id
...
az webapp auth update -g $RG -n $API --enabled true \
--action LoginWithAzureActiveDirectory \
--aad-allowed-token-audiences $WEBURL \
--aad-client-id $API_ID --aad-client-secret $SECRET  \
--aad-token-issuer-url $WEBURL